    ...
    def analyze(self, event):
        ...
        if self.is_mc:
            if self.MODE == "HWW" :
                ...
                isHWW = Process_1Lepton_GenMatching_H(self,event)
        ...
        # print("nUCSDLooseMu is ",nUCSDlooseMu) #Print for test.
        ...
        leading_muon_pt = 0
        for imu in range(0, event.nMuon):
            if (abs(muons[imu].eta)<2.4 and event.Muon_mediumId[imu] and event.Muon_corrected_pt[imu]>30 and abs(muons[imu].dz)<0.1 and abs(muons[imu].dxy)<0.05 and event.Muon_sip3d[imu]<=4.0 ):# to be checked
                # print("Muon pt of ",imu," is ",event.Muon_corrected_pt[imu])#print pt value for test.
                if(event.Muon_corrected_pt[imu]>=leading_muon_pt):
                    leading_moun_pt=event.Muon_corrected_pt[imu]
                    leading_muon_index = imu 
                muon_v4_temp.SetPtEtaPhiM(event.Muon_corrected_pt[imu], muons[imu].eta, muons[imu].phi, muons[imu].mass)
                UCSDgoodMuons.append(muon_v4_temp.Clone()) 
        nUCSDgoodMu = len(UCSDgoodMuons)
        ...
        if(nUCSDgoodMu>0):
            ...
            Selected_leading_muon= ( (event.Muon_corrected_pt[leading_muon_index]>30) and ( (event.Muon_corrected_pt[leading_muon_index]>=55 and event.Muon_miniPFRelIso_all[leading_muon_index]<0.2) or (event.Muon_corrected_pt[leading_muon_index]<55) ))
        ...
        UCSDlooseElectrons = []
        UCSDgoodElectrons = []
        UCSDleadingElectrons = []

        for iele in range(0, event.nElectron):
            if (event.Electron_cutBased[iele]>=2 and abs(electrons[iele].eta)<2.4 and (abs(electrons[iele].eta)<1.44 or abs(electrons[iele].eta)>1.57) and ((electrons[iele].pt>35 and event.Electron_pfRelIso03_all[iele]<0.25) or electrons[iele].pt>120 )):
        ...
        leading_ele_pt = 0
        for iele in range(0, event.nElectron):
            if (electrons[iele].pt>38 and abs(electrons[iele].eta)<2.4 and (abs(electrons[iele].eta)<1.44 or abs(electrons[iele].eta)>1.57) and abs(electrons[iele].dz)<0.1 and abs(electrons[iele].dxy)<0.05 and electrons[iele].sip3d<=4.0 and  electrons[iele].mvaFall17V2noIso_WP90):
                # print("Ele pt of ",iele," is ",event.Electron_pt[iele])#print pt value for test.
                if(electrons[iele].pt>=leading_ele_pt):
                    leading_ele_pt = electrons[iele].pt
                    leading_ele_index = iele 
                electron_v4_temp.SetPtEtaPhiM(electrons[iele].pt, electrons[iele].eta, electrons[iele].phi, electrons[iele].mass)
                UCSDgoodElectrons.append(electron_v4_temp.Clone())
        nUCSDgoodEle = len(UCSDgoodElectrons)
